...
- **Component Library** — Component table (syntax is **case-sensitive** in-app); **Import JSON component**; **DIY** overrides with unit suffixes (μm / °) — **channel** profile **CHANNEL** (`channelWidth`, `height`) or **ROUNDED CHANNEL** (`channelRadius`); **mux** seed is **Super_Mux_8** (8-input H MUX → 1 outlet; DIY **`leafPitch`**, **`valveWidthX`** / **`valveWidthY`**; legacy `spacing`/`width`/`valveWidth`/`length` migrate); mixer includes `edgeBend` / `edgeBend1` / `edgeBend2`; **Go to 3DuF** per row (requires device JSON with `components` / `layers`).
...
- **Bundled library seeds:** `Data/3DuF_component/default/{JSON,MINT,LFR}` ship P&R-style device JSON (not glyph-only `renderLayers` dumps) aligned with default MINT/LFR. Default boards are **40 000²** (tree/mux **80 000²**); FLOW `channelWidth` / channel **height** default **600**; mux is **Super_Mux_8** (`leafPitch=4000`, `valveWidthX=1800`, `valveWidthY=500`, 8→1 H MUX with I/O + control pads); mixer defaults include **edgeBend1/edgeBend2**; reaction chamber uses MINT/LFR entity **`REACTION CHAMBER`**; valve3D control ports use layer **CONTROL**.
