...
- **Component Library** — Component table (syntax is **case-sensitive** in-app); **Import JSON component**; **DIY** overrides — **channel** profile **CHANNEL** (`channelWidth`, `height`) or **ROUNDED CHANNEL** (`channelRadius`); mixer includes `edgeBend` / `edgeBend1` / `edgeBend2`; **Go to 3DuF** per row (requires device JSON with `components` / `layers`).
