...
- **Dashboard** — Workspaces and files. Click an LFR/MINT **file name** (or the pen icon) to open it in the Editor. Workspace cards show a **notes** control (read-only) when notes were set at create time. On **`.lfr` / `.mint` / `.json`** rows: **Copy** / **Move** to another workspace (or create one with notes); destination keeps the **same file name**; Move removes the original. JSON rows: **3DuF** is always left of **Save to component library**. **Export workspaces** / **Import workspaces** (.zip, includes jobs and the component-library cache).
- **Editor** — Monaco editor toolbar: **Save file**; **Save and synthesize** (full place-and-route → MINT + PR JSON + log/eval; appears on **Jobs** / **Alerts**); LFR-only **Compile to MINT** (writes `*_fromLFR.mint` into the workspace only — **no Jobs row**, no PR JSON); **Rename** (Confirm/Cancel; applies immediately, keeps last-saved body); **Move to another workspace** / **Copy to another workspace** (optional new workspace + notes; destination **file name stays the same**; stays in Editor after transfer); per-file **Import** (pick existing or new workspace + notes) / **Export** / **Delete**. Script language is fixed by the file extension (selector disabled). Unsaved buffer edits are discarded when you leave the Editor unless you used Save / Save and synthesize / Rename Confirm / Compile to MINT / Move-Copy. Ordinary **Save file** does **not** bump **Last Edited**; Rename, Move/Copy/Import destinations, and compile generate do. After **Save file**, Dashboard opens with that workspace expanded. Compiling `X.lfr` / `X_fromLFR.mint` never overwrites handwritten `X.mint`. LFR cross-file `` `import "WorkspaceName/file.lfr" `` (missing/circular imports show a dialog and block compile; empty stub `.lfr` modules are allowed); integrated **terminal**; **LLM prompts** sidebar **only** on this route. After a full reload, opening Editor still binds the Example workspace (so “Current workspace” is not None).
- **Jobs** — Full **Save and synthesize** runs only (**Compile to MINT** is omitted). Columns include **Input File** (stem), **Input Format**, **Workspace** (first three sticky while scrolling), scores, JSON/Log/Visualization, Status, **Delete**. Table auto-refreshes every **10 s**. **Apply** on evaluation weights recomputes **Total** for all rows without a new backend call. Compile outputs are written into the originating workspace (sidecars such as logs/eval stay on the job / backup zip). Reloading or reopening the site **clears jobs** with the rest of the guest session (they are not restored into Example).
- **Alerts** — Done/fail for **Save and synthesize** jobs; unread count on the bell; open the matching Jobs row.
- **Component Library** — Component table (syntax is **case-sensitive** in-app); **Import JSON component**; **DIY** overrides with unit suffixes (μm / °) and display order from **`src/lib/paramDisplayOrder.js`** (`componentSpacing` keepout last). **Channel type** **CHANNEL** (`channelWidth`, `height`) or **ROUNDED CHANNEL** (`channelRadius`; JSON `crossSection`; MINT `RoundedChannel=`); default seed is rounded. **mux** seed is **Super_MUX_8** (8-input H MUX → 1 outlet; DIY **`leafSpace`**, **`stageSpace`**, **`valveWidthX`/`valveWidthY`**; legacy `leafPitch`/`spacing`/`stageLength`/`width`/`length`/`valveWidth` migrate). **TREE** DIY uses the same **`leafSpace`/`stageSpace`** (BANK `spacing` stays instance pitch). Mixer includes `edgeBend` / `edgeBend1` / `edgeBend2` and defaults `channelWidth=600`, `bendSpacing=1400`, `bendLength=2000`, `numberOfBends=1`. LFR binary `+` already maps to MIXER (no `#MAP`); `#CONSTRAIN` writes those geometry keys. See **References** → Compiler Directives. Built-in default **`componentSpacing=1000`** µm keepout. MINT uses **`RoundedChannel=0|1`** (not `True`/`False`). **Go to 3DuF** per row (requires device JSON with `components` / `layers`).
- **References** — LFR / MINT language links into **Neptune_2026** docs on GitHub. Wiki mirrors use underscore paths: `docs/LFR_TestCases_wiki/` and `docs/MINT_TestCases_wiki/` (e.g. `Compiler_Directives.md`, `Fluidic_Operations.md` — not hyphenated `LFR-TestCases-wiki`). Also links the combined **LFR + MINT** manual and the V2 readable LFR spec. Pointers: **[docs/LFR_READABLE_SPEC_LOCATION.md](./docs/LFR_READABLE_SPEC_LOCATION.md)**.
...
| Optional local 3DuF (separate repo) | **8082** / **8083** | e.g. `http://localhost:8083` — see below |
...
Open-in-3DuF uses `THREE_DUF_APP_URL` in **[src/lib/open3DuFPostMessage.js](./src/lib/open3DuFPostMessage.js)**. **This checkout temporarily points at local 3DuF `http://localhost:8083/`** (production default is **[https://3duf.org/](https://3duf.org/)**). Neptune opens that URL in a new tab and sends the design JSON with `postMessage` (`loadDeviceFromJSON`).

Before send, Neptune **normalizes** library-scale device JSON (compact board **40 000²**, tree/mux **80 000²**, recenter when needed) via **[src/lib/normalizeDeviceJsonFor3DuF.js](./src/lib/normalizeDeviceJsonFor3DuF.js)**. Prefer `{ type: 'threeduf-ready' }` from 3DuF; timed fallbacks remain for older builds.
...
Open the URL printed by Vue CLI (often **`http://localhost:8082`**; this Neptune checkout expects **`http://localhost:8083/`** — match `THREE_DUF_APP_URL` to the printed port).

Then refresh (or rebuild) the GUI. More detail: [3DuF README](https://github.com/CIDARLAB/3DuF).
...
- **Compile modes:** **Save and synthesize** → `fluigi synthesize` (LFR) or `synthesizeFromMINT` (MINT) for full P&R (Jobs + Alerts). LFR **Compile to MINT** → `fluigi compile_lfr` with body `compileMode: "lfrToMint"` (alias `POST /api/v1/lfrToMint`); writes primary `*_fromLFR.mint` into the workspace only (**not** listed on Jobs). Compiling `X.lfr` / `X_fromLFR.mint` never overwrites handwritten `X.mint`. `--pre-load` is added **only** for Editor `` `import `` modules. **3DuF visualization JSON is not passed as `--component-library`** (that lookup hangs P&R on PORT/VALVE3D). Local and Modal fluigi wall clock is **600 s** (Modal function timeout **700 s**).
...
- **Bundled library seeds:** `Data/3DuF_component/default/{JSON,MINT,LFR}` ship P&R-style device JSON aligned with default MINT/LFR. Default boards are **40 000²** (tree/mux **80 000²**); default keepout **`componentSpacing=1000`**. Channel seed is **ROUNDED CHANNEL** / MINT **`RoundedChannel=1`** (FLOW width/height **600**). Mux is **Super_MUX_8** (`leafSpace=4000`, `stageSpace=4000`, `valveWidthX=1800`, `valveWidthY=1000`, flow/control widths **600**). Mixer defaults: `channelWidth=600`, `bendSpacing=1400`, `bendLength=2000`, `numberOfBends=1`, `edgeBend1/2=300`. TREE uses **`leafSpace`/`stageSpace`**. Reaction chamber uses MINT/LFR entity **`REACTION CHAMBER`**; valve3D control ports use layer **CONTROL**.
