...
- **Bundled library seeds:** `Data/3DuF_component/default/{JSON,MINT,LFR}` ship P&R-style device JSON aligned with default MINT/LFR. Default boards are **40 000²** (tree/mux **80 000²**); default keepout **`componentSpacing=2000`**. Channel seed is **ROUNDED CHANNEL** / MINT **`RoundedChannel=True`** (FLOW width/height **600**). Mux is **Super_MUX_8** (`leafSpace=4000`, `stageSpace=4000`, `valveWidthX=1800`, `valveWidthY=1000`, flow/control widths **600**). Mixer defaults: `channelWidth=600`, `edgeBend1/2=300`. TREE uses **`leafSpace`/`stageSpace`**. Reaction chamber uses MINT/LFR entity **`REACTION CHAMBER`**; valve3D control ports use layer **CONTROL**.
