...
Edit LFR in the Editor, use **Save and synthesize** (full place-and-route → **Jobs**) or **Compile to MINT** (MINT only → workspace file, **no Jobs row**).
...
Fluigi wall clock is **600 s** locally and on Modal (Modal function timeout **700 s**). Timeouts return an error log like `compile timed out after 600s (no results)`.
...
2. **Editor:** edit, then either:
   ...
   - **Compile to MINT** for LFR → MINT only (no PR JSON / no evaluation / **no Jobs row**).
...
4. On **Compile to MINT** success the workspace receives / opens `*_fromLFR.mint` (unplaced intermediate JSON is discarded). Built-in example seeds are **not** timestamp-stamped. Compiling `X.lfr` never overwrites handwritten `X.mint`.
5. **Jobs** / **Alerts** list only **Save and synthesize** runs (auto-refresh every 10 s). **Delete** on the job or the primary JSON removes the job and sibling generated files (JSON / MINT / log / eval).
...
| Action | Original file | Destination / navigation | Last Edited |
|--------|---------------|--------------------------|-------------|
| Save file | Updated in place | Same workspace; then Dashboard expands that workspace | **Not** bumped |
| Rename (Confirm) | Renamed immediately | Same workspace; keeps last-saved body | Bumped |
| Move to another workspace | Removed after a successful write | Existing or new workspace (+ notes); **file name unchanged**; stays in Editor | Destination bumped |
| Copy to another workspace | Unchanged | Existing or new workspace (+ notes); **file name unchanged**; stays in Editor | Destination bumped |
| Import | — | Pick existing or new workspace (+ notes) for the uploaded file | Destination bumped |

**Dashboard** also offers **Copy** / **Move** on `.lfr` / `.mint` / `.json` rows (same rules; no need to open Editor).
...
| Editor action | `compileMode` / type | Fluigi | Jobs? |
|---------------|----------------------|--------|-------|
| Save and synthesize (`.lfr`) | `lfr` | `fluigi synthesize -o <out> <source>` | Yes |
| Save and synthesize (`.mint`) | `mint` | `fluigi synthesizeFromMINT -o <out> <source>` — updates PR JSON / artifacts, **not** the source `.mint` | Yes |
| Compile to MINT (`.lfr` only) | `lfrToMint` (alias `POST /api/v1/lfrToMint`) | `fluigi compile_lfr -o <out> <source>` — success = primary `*_fromLFR.mint` | **No** |
...
- On synthesize / Compile to MINT, the Editor sends only the referenced files as `importLfr`. Missing paths or circular imports show a dialog and block compile before fluigi runs. Empty stub `.lfr` modules are still written into the compile tree.

### Example workspace

Guest **Example** seeds `flow_only_demo` and `flow_and_control_demo` (LFR, handwritten MINT, `*_fromLFR.mint`, PR JSON). Workspace notes explain the demos and that `*_fromLFR.mint` is compiler output while the plain `.mint` is handwritten. Missing seed files are recreated only if Example is empty; existing seed content/timestamps are not overwritten on sync.
...
- **Save:** **PUT /api/v1/file** should return 200. Optional body `forceTouch: true` bumps Last Edited (default Save does not).
- **Save and synthesize:** **POST /api/v1/mushroommapper** (LFR) or **POST /api/v1/fluigi** (MINT). Check **Jobs**.
- **Compile to MINT:** **POST /api/v1/mushroommapper** with `compileMode: "lfrToMint"` (or **POST /api/v1/lfrToMint**). Expect workspace `*_fromLFR.mint`, not a Jobs row.
- Confirm Neptune_2026 is at `NEPTUNE_2026_ROOT` and primitives are up. On timeout, see `compile timed out after 600s (no results)`.
